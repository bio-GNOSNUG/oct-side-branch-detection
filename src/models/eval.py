...
def main(config):
    ...
    with torch.no_grad():
        for i in tqdm(range(len(test_dataset))):
            ...
            # vessel_wal_path = config['DATA_ROOT'] + 'Vessel Wall Predictions/{}/{}/{}.npy'.format(target['image_id'][:-5], config['MODALITY'], target['image_id'][-4:])
            ...
            for box_i, box_pred in enumerate(pred['boxes']):
                #print(box_pred)
                bbox_mask = bbox_pascal_2_mask(box_pred.cpu().numpy().tolist(), 224)
                #intersection = calc_intersection(bbox_mask, lumen_mask)
                #print(intersection)
                #if intersection > 0.75 or intersection == 0:
